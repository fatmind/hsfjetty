//========================================================================
//$Id: AbstractJettyMojo.java 6311 2011-01-06 21:30:04Z jesse $
...
//    @Parameter(defaultValue = "D:\\env\\hsf\\1.4.9.6\\taobao-hsf.sar",alias = "hsf.home")
    @Parameter(alias = "hsf.home")
    private File hsfDirectory;
     ...
     * required
     */
    @Parameter(alias="jetty.scanIntervalSeconds",defaultValue="0")
     ...
     */
    @Parameter(alias="jetty.reload",defaultValue="manual")
     ...
     */
    @Parameter(alias="jetty.systemPropertiesFile")
    ...
    @Parameter(alias=PORT_SYSPROPERTY,defaultValue = "80")
     ...
     */
    @Parameter(alias="jetty.daemon",defaultValue="false")
    protected boolean daemon;
    
    /**  
     * parameter expression="${jetty.skip}" default-value="false"
     */
    @Parameter(alias="jetty.skip",defaultValue="false")
    ...
    public void setSystemProperties(SystemProperties systemProperties)
    {
        ...
        else
        {
            ...
            while (itor.hasNext())
            {
                ...
                this.systemProperties.addSystemProperty(prop);
            }   
        }
    }
    ...
    {try {
        ...
        getLog().info("begin to checkPomConfiguration()............");
...
//        replacer.replace();
        startJetty();
        getLog().info("sucessfully startJetty()!!!!!!!!!!!!");
    }catch (Exception e){
        new MojoFailureException("hsfjetty run goal failure!",e);
    }
    }
...
//            setSystemPropertiesFile(Resource.newClassPathResource("jetty-logging.properties").getFile());

            setSystemProperties(makeJettyLogProperties());
...
//            PlaceholderReplacer replacer = new AutoconfigReplacer(project,webap,)
    ...
    private SystemProperties makeJettyLogProperties() {
        SystemProperty logName = new SystemProperty();
        logName.setKey("org.eclipse.jetty.util.log.class");
        logName.setValue("org.eclipse.jetty.util.log.StdErrLog");
        SystemProperty logLevel = new SystemProperty();
        logLevel.setKey("org.eclipse.jetty.LEVEL");
        logLevel.setValue("INFO");
        SystemProperties logProp = new SystemProperties();
        logProp.setSystemProperty(logName);
        logProp.setSystemProperty(logLevel);
        return logProp;
    }

/*    public WebAppContext getHSFJettyWebAppContext(){
        HSFJettyWebAppContext hsfJettyCtx = new HSFJettyWebAppContext();
        hsfJettyCtx.setContextPath("/");
    ...
    public void configureWebApplication () throws Exception
    {
        ...
        if (webAppConfig == null)
            webAppConfig = new JettyWebAppContext(replacer);
        ...
        if (cp == null || "".equals(cp)||"/".equals(cp))
        {
        ...
        getLog().info("Web overrides = "+(webAppConfig.getOverrideDescriptors()==null?" none":webAppConfig.getOverrideDescriptors().toString()));
    }
...
//    @Parameter(defaultValue="${maven.war.webxml}",readonly=true)
     ...
     */
    @Parameter(defaultValue="C:/Users/haibo.lhb/antx.properties")
    ...
    @Parameter(defaultValue = "${project.build.directory}/hsf_jetty_temp")
    private File hsfJettyTempDirectory;
    ...
    public File getAutoconfigFile() {
        String[] autoconfigPath = new String[]{autoconfigFile.getAbsolutePath(),getWebAppSourceDirectory()+File.separator+"META-INF/autoconfig/auto-config.xml"
                ,getWebAppSourceDirectory()+File.separator+"META-INF/autoconf/auto-config.xml"};
        for (String path : autoconfigPath) {
            autoconfigFile = new File(path);
            if(autoconfigFile == null || !autoconfigFile.exists()){
                getLog().error(autoconfigFile.getAbsolutePath()+", autoconfig file not exists!");
            }else{
                getLog().info(autoconfigFile.getAbsolutePath()+", autoconfig file exists!");
    ...
    public String getCurrentPluginArtifactId() {
        return currentPluginArtifactId;
    }
    ...
    public File getHsfJettyTempDirectory() {
        return hsfJettyTempDirectory;
    }
}
